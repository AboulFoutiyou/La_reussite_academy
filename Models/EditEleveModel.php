...
class EditEleveModel {
    ...
    public function updateEleve($id, $eleveData) {
        $allowedFields = ['matricule', 'nom', 'prenom', 'date_naissance', 'nom_tuteur', 'departement', 'classe'];
        $updates = [];
        $params = [':id' => $id];
    ...
    public function getEleveById($id) {
        $sql = "SELECT id, matricule, nom, prenom, date_naissance, nom_tuteur, departement, classe
                FROM eleves
                WHERE id = :id";
